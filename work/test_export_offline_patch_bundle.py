...
class ExportOfflinePatchBundleTests(unittest.TestCase):
    def test_export_matches_player_facing_vf2_style_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            output = Path(temp) / "Fish-Tycoon-Fix-Patcher-v1.2.3"
            ...
            self.assertEqual(manifest["patched_sha256"], "C0D664D79748F72B9D8183F87BAB1B2CA969C7647E1EBA427256D4EF4C6D2366")

    def test_zip_has_one_top_level_folder_and_no_game_executable(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            output = root / "Fish-Tycoon-Fix-Patcher-v1.2.3"
            archive = root / "Fish-Tycoon-Fix-Patcher-v1.2.3.zip"
