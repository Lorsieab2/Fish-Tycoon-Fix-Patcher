...
class ExportOfflinePatchBundleTests(unittest.TestCase):
    def test_export_matches_player_facing_vf2_style_contract(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            output = Path(temp) / "Fish-Tycoon-Fix-Patcher-v1.2.2"
            ...
            self.assertEqual(manifest["version"], "v1.2.2")
            self.assertEqual(manifest["patched_sha256"], "8F458EE27E0B4BF7DE8CF3D125F506F3D78993F77135DE23019E2B1F2EB6876B")

    def test_zip_has_one_top_level_folder_and_no_game_executable(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            output = root / "Fish-Tycoon-Fix-Patcher-v1.2.2"
            archive = root / "Fish-Tycoon-Fix-Patcher-v1.2.2.zip"
