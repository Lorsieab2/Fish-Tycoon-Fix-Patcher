...
def save_paths(values: dict[str, str]) -> None:
...
class FishTycoonPatcherGUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        saved = load_paths()
        self.game_dir = tk.StringVar(value=saved.get("game_dir", ""))
    ...
    def _build(self) -> None:
        ...
        paths.pack(fill="x")
        self._path_row(paths, 0, "Vanilla game folder", self.game_dir, self.browse_game)
        self._path_row(paths, 1, "Patch manifest", self.manifest, self.browse_manifest)
        self._path_row(paths, 2, "Bug-fixed output folder", self.output_dir, self.browse_output)
        self._path_row(paths, 3, "Backup folder (restore)", self.backup_dir, self.browse_backup)
    ...
    def _path_row(self, parent: tk.Widget, row: int, label: str, value: tk.StringVar, command: object) -> None:
    ...
    def start_apply(self, dry_run: bool) -> None:
        ...
        self.run_worker("Dry run" if dry_run else "Apply", lambda: patcher.apply_manifest(args))
    ...
    def run_worker(self, label: str, operation: object) -> None:
        self.busy = True
        ...
        def worker() -> None:
            ...
            self.root.after(0, lambda: self.finish_worker(label, result, stream.getvalue(), error))

        threading.Thread(target=worker, daemon=True).start()

    def finish_worker(self, label: str, result: int, text: str, error: str) -> None:
